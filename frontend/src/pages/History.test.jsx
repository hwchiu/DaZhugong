...
describe('History page', () => {
  ...
  describe('特殊Token(隱藏摩擦豬公彩蛋)顯示規則', () => {
    it('一般Token顯示+1 Token，不再是寫死的1 Token', () => {
      useGroupMock.mockReturnValue({
        members: [
          { id: 'reporter', name: '阿明', active: true },
          { id: 'target', name: '小美', active: true },
      ...
      useTokensMock.mockReturnValue({
        tokens: [{ id: 'normal-1', reporterId: 'reporter', targetId: 'target', reason: '討論會議', timestamp: Date.UTC(2026, 7, 23) }],
        loading: false,
        error: null,
      });

      render(<History />);
      const row = screen.getByRole('list', { name: '已確認 Token 歷史紀錄' }).querySelector('li');
      expect(row.textContent).toContain('+1 Token');
      expect(row.textContent).not.toContain('特殊');
    });

    it('spec 13.1：特殊Token顯示「（特殊）」標記、+5 Token、5x徽章，不顯示成5顆一般Token', () => {
      useGroupMock.mockReturnValue({
        members: [
          { id: 'reporter', name: '阿明', active: true },
          { id: 'target', name: '小美', active: true },
      ...
      render(<History />);
      const row = screen.getByRole('list', { name: '已確認 Token 歷史紀錄' }).querySelector('li');
      expect(row.textContent).toContain('（特殊）');
      expect(row.textContent).toContain('+5 Token');
      expect(row.textContent).toContain('5x');
      // 畫面上只有「一筆」列表項目，不是5個獨立的一般Token項目
      const rows = screen.getByRole('list', { name: '已確認 Token 歷史紀錄' }).querySelectorAll('li');
