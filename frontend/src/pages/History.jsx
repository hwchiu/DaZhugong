          ...
          <>
            ...
            <ol aria-label="已確認 Token 歷史紀錄" className="space-y-3">
              {rows.map((token) => {
                ...
                // 特殊Token(隱藏摩擦豬公彩蛋)：spec 13.1的History Display Rule——
                // 「顯示1顆特殊Token，但帳面+5」，畫面上要同時看得出「這是特殊的」
                // 跟「它值多少」，不能因為只有1筆(displayTokenCount=1)就跟一般Token
                // 顯示成一樣的樣子，也不能為了強調它價值5就畫成5顆一般Token。
                    ...
                    <article>
                      {isSpecial ? (
                        <p className="mb-2 inline-flex items-center gap-1 rounded-full bg-rose-600 px-2.5 py-1 text-xs font-bold text-white">
                          <span aria-hidden="true">⭐</span> 特殊 Token · 5x
                        </p>
                      ) : null}
                      <p className="text-base leading-7 text-slate-800">
                        ...
                        {isSpecial ? <span className="ml-1 text-sm font-semibold text-rose-600">（特殊）</span> : null}
                      </p>
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-100 pt-3">
                        ...
                        <span
                          className={`shrink-0 rounded-full px-3 py-1 text-sm font-bold text-white ${isSpecial ? 'bg-rose-600' : 'bg-slate-950'}`}
                        >
                          +{tokenValue} Token
                        </span>
                      </div>
